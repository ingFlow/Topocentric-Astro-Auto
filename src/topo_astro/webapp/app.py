...
Recent changes:
    - De-duplication phase: this file no longer defines its own local
      copies of parse_selection_file and get_technique_name - both were
      imported directly from constants.py, which already defined the
      canonical versions. The only previously-forked functional
      differences were: an extra debug print() statement, one extra
      (redundant, given well-formed real data) whitespace guard on the
      aspect-line check, and one missing log line for the file-not-found
      case - none of which affect the parsed result for any real
      saved_selections file, confirmed directly by diffing both
      implementations before removing this one.
