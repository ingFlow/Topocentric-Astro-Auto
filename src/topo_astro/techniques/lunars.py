"""
techniques/lunars.py - the Lunar (cyclic lunar returns) technique:
Lunar_Auto.

Structurally different from every other technique in this package: one
instantiation computes ALL THREE lunar types (LunarType.LUNAR, KINETIC,
AS_LUNAR) internally, each of which can itself expand into a direct/demi-
direct/converse/demi-converse sub-chart, and accumulates every resulting
sub-chart into one combined list (get_all_lunars()). Every other technique
in this package computes exactly one calculation per instantiation. This
is why get_all_lunars()/get_info() have their own shapes and names rather
than matching the (direct, converse) pair + get_info() pattern the other
six techniques share - a permanent, technique-intrinsic fact, not an
inconsistency to unify away.

Recent change (de-duplication/mechanical-move phases): fixed
get_str_only_aspects_from_data's argument-count mismatch (it called
Lunar_Auto with a stray extra `ltype` argument the real constructor never
accepted) - this is now a working, first-class alternative to
get_str_labelled_aspects_from_array, returning a flattened, unlabeled
aspect string instead of the labeled, per-lunar-return-type one that
get_all_lunars()/get_str_labelled_aspects_from_array already provide.
"""
