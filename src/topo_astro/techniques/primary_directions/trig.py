"""
techniques/primary_directions/trig.py - the spherical-trigonometry core
of the Primary Directions technique: PD_Base (the class doing the actual
MD/SA/OA-OD arc math and the MD > SA quadrant-shift correction) plus its
supporting helpers (calc_RA_DECL, calculate_ADP, calculate_OA_OD,
calc_md_to_oa_data, calc_arc, and the house/quadrant helpers).

Convention note (original): there is no difference between calculating
the north and south MD-to-OA data other than that, for southern
geographic latitudes, the calculation uses the absolute value of the
latitude - these results are validated against POLARIS.

See techniques/primary_directions/technique.py for the PD_Automate class
that orchestrates this trig core against a real radix/event pair, and
techniques/primary_directions/reserve.py for a preserved, currently-unwired
alternate entry point (get_directed_from_data) built on top of the same
