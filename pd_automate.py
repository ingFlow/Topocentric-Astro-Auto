...
date_hour, date_minute, date_second = 17, 24, 13
JD_RADIX = swe.julday(1940, 10, 9, date_hour + date_minute/60 + date_second/3600)
geopos = [53.4, -2.9833333, 70.0]
geo_latitude = geopos[0]
geo_longitude = geopos[1]
jd_event = swe.julday(1975, 10, 9)
ARC_P = pd.calc_arc(JD_RADIX, jd_event)
ORB = 0.33

def calc_all_pd_houses():
...
def calc_all_pd_planets():
    ...
    for i in range(0, len(PLANETS)):
        ...
        ac, mc, ramc = calc_radix_ac_mc_ramc()
...
def calc_directed_POF(rad_planets):
    ...
    pof_long = ac + long_moon - long_sun
...
#jd_event = swe.julday(1962, 8, 23)

#calc_directed_planet(jd_event, geopos, 6, False)
rad_houses, dir_houses, conv_houses = calc_all_pd_houses()
rad_planets, dir_planets, conv_planets = calc_all_pd_planets()
#add POF DATA
rad_pof, dir_pof, conv_pof = calc_directed_POF(rad_planets)
rad_planets.append(('POF', rad_pof, "(r)"))
dir_planets.append(('POF', dir_pof, "(d)"))
conv_planets.append(('POF',conv_pof, "(c)"))
#JOIN ARRAYS
rad_positions = [*rad_planets, *rad_houses[0]]
dir_positions = [*dir_planets, *dir_houses[0]]
conv_positions = [*conv_planets, *conv_houses]
# Modify arrays with their corresponding suffixes
'''rad_positions = add_suffix_to_tuples(rad_positions, "(r)")
dir_positions = add_suffix_to_tuples(dir_positions, "(d)")
conv_positions = add_suffix_to_tuples(conv_positions, "(c)")'''

str_aspects_rad_dir = aspects.find_trans_aspects(rad_positions, dir_positions, ORB)
str_aspects_rad_conv = aspects.find_trans_aspects(rad_positions, conv_positions, ORB)
str_aspects_rad_conv = aspects.remove_duplicates(str_aspects_rad_conv)
str_aspects_rad_dir = aspects.remove_duplicates(str_aspects_rad_dir)

with open("swe_ephem_output.txt", "w") as file:
    file.write(f"Event Date: {str(julian.from_jd(jd_event))}, \tARC: {pssr.convert_dec_degrees_to_deg_min_sec(ARC_P)} \nRad Positions: {rad_planets}{rad_houses} \nDir Positions: {dir_planets}{dir_houses} \nCon Positions: {conv_planets}{conv_houses}")
    file.write(f"\nRAD-DIR ASPECTS: \n{str_aspects_rad_dir}")
