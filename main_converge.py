...
def convert_birth_data_json(file_to_write_str):
    ...
    list_of_events = [
        #(datetime(2004, 12, 15, 12, 00, 00),pd.EventType.MOVE_HOME,[26.17678333, 28.04259444, 1753.0]),
        (datetime(2009, 3, 19, 12, 00, 00),pd.EventType.BIRTH_BROTHER,[26.17678333, 28.04259444, 1753.0]),
        #(datetime(2011, 1, 20, 12, 00, 00),pd.EventType.SUCCESS,[26.17678333, 28.04259444, 1753.0]),
        (datetime(2013, 12, 11, 12, 00, 00),pd.EventType.LOSSES,[26.17678333, 28.04259444, 1753.0]),
        (datetime(2014, 3, 24, 12, 00, 00),pd.EventType.BIRTH_BROTHER,[26.17678333, 28.04259444, 1753.0]),
        (datetime(2015, 5, 27, 12, 00, 00),pd.EventType.LOSSES,[26.17678333, 28.04259444, 1753.0]),
        (datetime(2017, 9, 15, 12, 00, 00),pd.EventType.SUCCESS,[26.17678333, 28.04259444, 1753.0]),
        (datetime(2018, 2, 19, 12, 00, 00),pd.EventType.BIRTH_SISTER,[26.17678333, 28.04259444, 1753.0]),
        (datetime(2019, 1, 8, 12, 00, 00),pd.EventType.SUCCESS,[26.17678333, 28.04259444, 1753.0]),
        (datetime(2019, 5, 22, 12, 00, 00),pd.EventType.LOSSES,[26.17678333, 28.04259444, 1753.0]),
        (datetime(2019, 8, 13, 12, 00, 00),pd.EventType.LOSSES,[26.17678333, 28.04259444, 1753.0]),
        (datetime(2021, 4, 6, 12, 00, 00),pd.EventType.LOSSES,[26.17678333, 28.04259444, 1753.0]),
        (datetime(2021, 7, 10, 12, 00, 00),pd.EventType.TRAVEL_NEGATIVE,[26.17678333, 28.04259444, 1753.0]),
        #(datetime(2021, 8, 15, 12, 00, 00),pd.EventType.LOSSES,[-4.05466, 39.66359, 23.0]),
        (datetime(2022, 1, 3, 12, 00, 00),pd.EventType.INTRIGUE,[-4.05466, 39.66359, 23.0]),
        (datetime(2022, 5, 25, 12, 00, 00),pd.EventType.TRAVEL_NEGATIVE,[-4.05466, 39.66359, 23.0]),
    ]
    ...
    data = {
        ...
        "geopos_natal": geopos,
        "list_of_events": [
            {"datetime": event[0].isoformat(), "event_type": pd.EventType.get_name(event[1]), "geopos": event[2]} for event in list_of_events
        ]
    }
